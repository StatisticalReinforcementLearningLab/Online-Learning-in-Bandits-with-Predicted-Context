...
class LinBandit:
    def __init__(self, theta = None, n_action = None, gamma = 0.95):
    ...
    def generate_potential_reward_history_list(self, T, user_id, sigma_e, sigma):
        ...
        x_tilde_list['dosage'] += np.random.normal(scale=sigma_e**(0.5), size = self.T) # add noise to dosage
    ...
    def TS_w_predicted_state(self, rho2, l = 1., p_0 = 0.1, decay_cut = 0.0):
        ...
        while t < T:
            ...
            regret_t = np.max([rt_0, rt_1]) - pi_0 * rt_0 - (1-pi_0)*rt_1
            regret = regret + regret_t  # update cumulative regret
            regret_list[t] = regret
            
            burden = self.gamma *  burden + at # update burden
